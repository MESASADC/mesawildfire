...
# Start Postgresql:
docker run --name supervisor_postgis -a stdout -a stderr --rm -v $MESA_ROOT/volumes/postgis:/var/lib/postgresql -v $MESA_ROOT/volumes/postgis/9.4/main/pg_hba.conf:/etc/postgresql/9.4/main/pg_hba.conf -p 65432:5432 kartoza/postgis
