...
# Start django:
docker run -t --rm -a stdout -a stderr --name supervisor_mesa_backend --link supervisor_postgis:postgis --link supervisor_rabbitmq:rabbitmq -v $MESA_ROOT/admin/ENV:/ENV -v $MESA_ROOT/django_project/start.sh:/start.sh -v $MESA_ROOT/django_project:/django_project mesasadc/mesa_django:$BUILD_TAG /django_project/manage.py mesa_comms
