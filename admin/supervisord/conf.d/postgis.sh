...
# Start Postgresql:
docker run --name supervisor_postgis -a stdout -a stderr --rm -e POSTGIS_ENABLE_OUTDB_RASTERS=1 -e POSTGIS_GDAL_ENABLED_DRIVERS=ENABLE_ALL -v $MESA_ROOT/admin/postgresql/pg_hba.conf:/etc/postgresql/9.4/main/pg_hba.conf -v $VOLUMES/postgis:/var/lib/postgresql -v $MESA_ROOT/ingest:/mnt/ingest -v $MESA_ROOT/admin/postgresql/start-postgis.sh:/start-postgis.sh -p 5432:5432 mesasadc/postgis /start-postgis.sh
RESULT=$?
echo RESULT=$RESULT
