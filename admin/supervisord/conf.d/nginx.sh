...
docker run --link supervisor_geonode --rm martin/wait
docker run --link supervisor_mesa_web --rm martin/wait

# Start django:
docker run -t --rm -a stdout -a stderr --name supervisor_nginx --link supervisor_geonode:geoserver --link supervisor_mesa_web:mesa_web --link supervisor_rabbitmq:rabbitmq -p 80:80 -v $MESA_ROOT/nginx/sites:/etc/nginx/sites-enabled -v $MESA_ROOT/nginx/www:/var/www/ mesa/nginx
