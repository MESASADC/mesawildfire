...
echo "Start django:"
docker run -t --rm -a stdout -a stderr --name supervisor_mesa_web --link supervisor_geonode:geoserver --link supervisor_postgis:postgis --link supervisor_rabbitmq:rabbitmq -v $MESA_ROOT/django_project/ENV:/ENV -v $MESA_ROOT/django_project/start.sh:/start.sh -v $MESA_ROOT/django_project:/django_project -v $MESA_ROOT/volumes/web_static:/static_root/ -p 8112:8000 mesa_django
